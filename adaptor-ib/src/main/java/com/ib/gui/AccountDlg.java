...
public class AccountDlg extends JDialog {
	...
	void updatePortfolio(Contract contract, double position, double marketPrice, double marketValue, double averageCost,
		...
		m_portfolioModel.updatePortfolio(contract, position, marketPrice, marketValue, averageCost, unrealizedPNL,
	...
	void accountDownloadEnd(String accountName) {
		...
		if (!Util.StringIsEmpty(m_accountName) && !m_accountName.equals(accountName)) {
			return;
		}
...
class PortfolioTable extends AbstractTableModel {
	...
	void updatePortfolio(Contract contract, double position, double marketPrice, double marketValue, double averageCost,
			double unrealizedPNL, double realizedPNL, String accountName) {
		PortfolioTableRow newData = new PortfolioTableRow(contract, position, marketPrice, marketValue, averageCost,
	...
	class PortfolioTableRow {
		...
		PortfolioTableRow(Contract contract, double position, double marketPrice, double marketValue,
...
class AcctValueModel extends AbstractTableModel {
	...
	void updateAccountValue(String key, String val, String currency, String accountName) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (test.m_key != null && test.m_key.equals(newData.m_key) && test.m_currency != null
